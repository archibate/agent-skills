...
LAUNCHER="$ROOT/skills/fable-advisor/scripts/ask-fable"
REAL_CLAUDE=$(command -v claude || true)
TEST_TMP=$(mktemp -d "${TMPDIR:-/tmp}/fable-advisor-smoke.XXXXXX")
...
DEFAULT_DENIED_PATH="$HOME/.npm/_logs/fable-advisor-probe-$RANDOM-$$"
...
if [[ -z "$REAL_CLAUDE" || ! -x "$REAL_CLAUDE" ]]; then
    printf 'fable-advisor smoke test: claude is required\n' >&2
    exit 127
fi
for dependency in curl jq; do
    if ! command -v "$dependency" >/dev/null 2>&1; then
        printf 'fable-advisor smoke test: %s is required\n' "$dependency" >&2
        exit 127
    fi
done
...
curl -sS --connect-timeout 3 --max-time 5 -o /dev/null https://example.com || {
    printf 'fable-advisor smoke test: host network control could not reach example.com\n' >&2
    exit 1
}

printf -v BASH_COMMAND_TO_TEST 'if : > %s 2>/dev/null; then write_blocked=0; else write_blocked=1; fi; if : > %s 2>/dev/null; then default_write_blocked=0; else default_write_blocked=1; fi; printf "%%s\\n" %s >"$TMPDIR/bash-proof"; bash_proof=$(cat "$TMPDIR/bash-proof"); if ! curl -fsS --connect-timeout 3 --max-time 5 https://example.com >/dev/null 2>&1; then network_blocked=1; else network_blocked=0; fi; if [[ -z ${FABLE_ADVISOR_BOUNDARY_SECRET+x} ]]; then credential_blocked=1; else credential_blocked=0; fi; printf "WRITE_BLOCKED=%%s\\nDEFAULT_WRITE_BLOCKED=%%s\\nBASH=%%s\\nNETWORK_BLOCKED=%%s\\nCREDENTIAL_BLOCKED=%%s\\n" "$write_blocked" "$default_write_blocked" "$bash_proof" "$network_blocked" "$credential_blocked"' \
...
OUTPUT=$(
    REAL_CLAUDE="$REAL_CLAUDE" \
        ...
        FABLE_ADVISOR_BOUNDARY_SECRET="$SECRET_TOKEN" \
        FABLE_ADVISOR_TIMEOUT_SECONDS=${FABLE_ADVISOR_TIMEOUT_SECONDS:-90} \
...
if (( STATUS != 0 )); then
    printf 'fable-advisor smoke test: launcher exited with status %s\n' "$STATUS" >&2
    exit "$STATUS"
fi
if [[ -e "$DENIED_PATH" ]]; then
    printf 'fable-advisor smoke test: Bash wrote outside its scratch directory\n' >&2
    exit 1
fi
if [[ -e "$DEFAULT_DENIED_PATH" ]]; then
    printf 'fable-advisor smoke test: Bash wrote to a denied Claude default path\n' >&2
    exit 1
fi
if ! awk '
    ...
    printf 'fable-advisor smoke test: wrapper did not select Haiku at low effort\n' >&2
    exit 1
fi
...
if ! jq -se --arg read_token "$READ_TOKEN" --arg bash_token "$BASH_TOKEN" '
    ...
    printf 'fable-advisor smoke test: tool evidence did not prove the security boundary\n%s\n' "$OUTPUT" >&2
    exit 1
fi

printf 'fable-advisor smoke test passed (Haiku/low): native Read, scratch-only Bash, blocked Bash network and credentials\n'
